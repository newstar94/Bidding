...
function transactionMarkup(transactions) {
  ...
  return transactions.map((transaction) => (
    `<div>${adminStatusMarkup(transaction.status)} <span class="text-secondary">· ${text(transaction.type)}</span><div class="small text-secondary">Mã thanh toán: <code>${text(transaction.id, transaction.providerTransactionId)}</code> · ${escapeHtml(formatMinorMoney(transaction.verifiedPaidAmountMinor, transaction.currency))} · ${formatDate(transaction.createdAt)}</div>${transaction.invoiceRequest ? `<div class="small text-secondary">Hóa đơn: ${text(transaction.invoiceRequest.id)} · ${adminStatusMarkup(transaction.invoiceRequest.status)}</div>` : ""}</div>`
  )).join("");
}
...
export function paymentDetailMarkup(payment) {
  ...
  const invoiceMarkup = invoiceLinks.length ? invoiceLinks.map((invoice) => `<dt class="col-5">Hóa đơn</dt><dd class="col-7">${text(invoice.id)} · ${adminStatusMarkup(invoice.status)}</dd>`).join("") : "<dt class=\"col-5\">Hóa đơn</dt><dd class=\"col-7\">Chưa có yêu cầu hóa đơn</dd>";
