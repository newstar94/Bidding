...
import { formatMinorMoney, requestAdminValue } from "./AdminBilling.js";
...
function sessionActivityMarkup(items) {
  if (!Array.isArray(items) || items.length === 0) return '<p class="text-secondary">Chưa có hoạt động đăng nhập.</p>';
  return `<ul class="list-group list-group-flush">${items.map((item) => {
    const context = [item.activeRole, item.activeRoleOrganizationId].filter(Boolean).map(text).join(" · ");
    const remembered = item.rememberMe ? " · Ghi nhớ đăng nhập" : "";
    return `<li class="list-group-item px-0"><strong>${text(item.status)}</strong><div class="small text-secondary">Gần nhất ${formatDate(item.lastSeenAt)} · Tạo ${formatDate(item.createdAt)}${remembered}</div>${context ? `<div class="small text-secondary">${context}</div>` : ""}</li>`;
  }).join("")}</ul>`;
}
...
function linksMarkup(links) {
  const labels = { sessions: "Phiên đăng nhập", subscription: "Đăng ký", invoices: "Hóa đơn", usage: "Sử dụng", audit: "Nhật ký", users: "Người dùng", activity: "Hoạt động", security: "Bảo mật" };
