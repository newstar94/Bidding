...
const authenticatedMutationEnabled = ['sync', 'upload']
  .some((name) => scenarioEnabled(name));
...
for (let index = 0; index < sessions.length; index += 1) {
  ...
  const cookieCsrfMatch = String(session.cookie).match(/(?:^|;\s*)csrf_token=([^;]+)/);
  if (authenticatedMutationEnabled && !session.csrfToken && !cookieCsrfMatch) {
    fail(`sessions[${index}].csrfToken is required for authenticated mutations`);
  }
...
function csrfTokenForSession(session) {
  const explicit = String((session && session.csrfToken) || '').trim();
  if (explicit) return explicit;
  const match = String((session && session.cookie) || '').match(/(?:^|;\s*)csrf_token=([^;]+)/);
  return match ? match[1].trim() : '';
}

function cookieHeader(session) {
  const raw = String((session && session.cookie) || '').trim();
  let header = raw.includes('=') ? raw : `session_token=${raw}`;
  const csrfToken = csrfTokenForSession(session);
  if (csrfToken && !/(?:^|;\s*)csrf_token=/.test(header)) {
...
function authParams(session, tags, extraHeaders, extraOptions) {
  ...
  if (csrfToken) authenticationHeaders['X-CSRF-Token'] = csrfToken;
...
export function loginFlow() {
  ...
  const csrfValues = response.cookies && response.cookies.csrf_token;
  if (response.status === 200 && cookieValues && cookieValues.length > 0
      && csrfValues && csrfValues.length > 0) {
    const csrfToken = csrfValues[0].value;
    const logout = http.post(
      `${baseUrl}/api/auth/logout`,
      null,
      {
        headers: {
          Cookie: `session_token=${cookieValues[0].value}; csrf_token=${csrfToken}`,
          Origin: baseUrl,
