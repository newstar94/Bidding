...
def get_active_org(request, user_id):
    ...
    personal_scope_allowed = bool(
        account_row and str(account_row[0] or "").strip().lower() != "super_admin"
    )
    cursor.execute("""
    ...
    if active_org == implicit_scope_id or not rows:
        if not personal_scope_allowed:
            exc = OrgPermissionError("Tài khoản quản trị nền tảng không có không gian cá nhân!")
            with _org_cache_lock:
                _org_cache[cache_key] = (exc, now + ORG_CACHE_TTL)
            raise exc
        if active_org and active_org != implicit_scope_id:
            exc = OrgPermissionError("Không có quyền truy cập phạm vi dữ liệu này!")
            with _org_cache_lock:
                _org_cache[cache_key] = (exc, now + ORG_CACHE_TTL)
            raise exc
