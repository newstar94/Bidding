...
def contract_tool_definitions() -> list[dict]:
    aggregate = {
        ...
        "properties": {
            ...
            "statuses": {
                ...
                "items": {"type": "string"},
                "maxItems": 12,
                "description": "Danh sách nhãn trạng thái đúng như trong workspace; nhãn có thể do người dùng tự đặt.",
            },
    ...
    listing = {"type": "object", "properties": {"dateField": {"type": "string", "enum": ["signed_date", "liquidation_date"]}, "dateFrom": {"type": ["string", "null"]}, "dateTo": {"type": ["string", "null"]}, "status": {"type": "string", "description": "Nhãn trạng thái đúng như trong workspace; nhãn có thể do người dùng tự đặt."}, "limit": {"type": "integer", "minimum": 1, "maximum": 20}}, "required": ["dateField", "dateFrom", "dateTo", "status", "limit"], "additionalProperties": False}
    return [
