...
def _is_short_initialism(word):
    letters = "".join(char for char in word.casefold() if char.isalpha())
    return 1 < len(letters) <= 4 and not any(char in VIETNAMESE_VOWELS for char in letters)
...
def normalize_procurement_partner_name(value):
    ...
    if not text.isupper():
        return re.sub(
            r"[^\W\d_][^\W_]*",
            lambda match: match.group().upper() if _is_short_initialism(match.group()) else match.group(),
            text,
        )

    def word_case(match):
        word = match.group()
        if word in ACRONYMS or _is_short_initialism(word) or any(char.isdigit() for char in word):
            return word
