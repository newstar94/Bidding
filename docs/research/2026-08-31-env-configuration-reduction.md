...
Tổng số key trong 23 nhóm baseline là 313, khớp các dòng active `KEY=VALUE` đã được chuyển sang `deploy/environment-variables.reference`. Các dòng provider-specific đang comment (Anthropic/Gemini/Ollama/Azure), CA file, metrics bearer token và credential backup/worker là catalog/profile tham khảo, không nằm trong con số 313.
