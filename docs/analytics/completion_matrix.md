...
| Global filters and shareable URL | Done | `frontend/admin-platform/AdminAnalytics.js`, `query_service.py` | Inclusive date range bounded to 366 days; release/owner/plan/variant/size/paid-state filters. |
...
| Bounded aggregate-only API and isolated admin surface | Done | `query_service.py`, `frontend/admin-platform/AdminAnalytics.js` | Secure build includes the analytics view only in the dedicated Platform Admin entry; aggregate API payloads remain bounded. |
