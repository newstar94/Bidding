...
const SAMPLE_COUNT = Math.max(1, Number(process.env.TABLE_BENCHMARK_SAMPLES || 5));
...
const scenarios = Object.freeze({
  packageGoods: {
    columns: 7,
    rowHeight: 72,
    productionStrategy: "full",
    interactionConstraint: "editable rows use delegated package-goods commands",
  },
  bidderGoods: {
    columns: 19,
    rowHeight: 80,
...
async function measure(page, { scenario, rowCount, strategy, options }) {
  return page.evaluate(async ({ scenarioName, count, renderStrategy, virtualOptions }) => {
    ...
    const tasks = globalThis.__tableBenchmarkLongTasks.splice(0);
    return {
...
try {
  ...
  await page.evaluate(async () => {
    [globalThis.__tableBenchmarkVirtual, globalThis.__tableBenchmarkTrusted] = await Promise.all([
  ...
  for (const [scenario, config] of Object.entries(scenarios)) {
    ...
    for (const rowCount of rowCounts) {
      ...
      for (let sample = 0; sample < SAMPLE_COUNT; sample += 1) {
        fullSamples.push(await measure(page, {
          scenario,
          rowCount,
          strategy: "full",
          options: {},
        }));
        virtualSamples.push(await measure(page, {
          scenario,
          rowCount,
          strategy: "virtual",
          options: {
