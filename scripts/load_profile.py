...
def validate_runtime_inputs(
    ...
    *,
    ...
    requires_csrf = any(
        scenarios[name]["enabled"] for name in ("sync", "upload")
    )
    sessions = []
    if requires_session:
        ...
        for index, session in enumerate(sessions):
            ...
            csrf_token = str(session.get("csrfToken") or "").strip()
            cookie_has_csrf = any(
                part.strip().startswith("csrf_token=")
                and part.strip() != "csrf_token="
                for part in cookie.split(";")
            )
            if requires_csrf and not csrf_token and not cookie_has_csrf:
                raise LoadProfileError(
                    f"sessions[{index}].csrfToken is required for authenticated mutations"
                )
