...
// BiddingController.ensureWorkflowModules() loads both workflow roots before
// workflow commands can run, so the workspace budget is their deduplicated
// static graph together with the authenticated bootstrap graph.
const authenticatedWorkspaceRoots = [
...
for (const key of authenticatedWorkspaceRoots) {
  if (!manifest[key]?.isDynamicEntry) {
    throw new Error(`Missing authenticated workspace entry in ${manifestPath}: ${key}`);
  }
}
