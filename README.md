...
*   **Thư mục phát triển (Source Code)**:
    *   `/controllers/`: Logic điều khiển nghiệp vụ Frontend (sạch, dễ đọc).
    *   `/views/`: Layout HTML và View rendering.
    *   `/models/`: Tương tác cơ sở dữ liệu local (IndexedDB).
...
3.  Ở chế độ này, server Python sẽ load trực tiếp các file gốc `/controllers/app.js` để bạn dễ dàng chỉnh sửa và debug trên DevTools trình duyệt (không bị nén hay xáo trộn).
...
1.  **Chạy lệnh đóng gói mã nguồn**:
    ...
    npm run build
    ```
    *Vite sẽ tự động quét file `controllers/app.js`, gom tất cả mã nguồn liên quan và tạo ra file bundle nén tại `/dist/controllers/app.bundle.js`.*
...
2.  **Cấu hình môi trường Production**:
    ...
    APP_DEBUG=False
    ```
3.  **Kết quả**: Server Python sẽ tự động thay thế liên kết script module gốc thành file đóng gói `/dist/controllers/app.bundle.js`. Người dùng truy cập ngoài internet sẽ chỉ thấy một file JS duy nhất đã được thu gọn, xáo trộn tên biến và không thể xem được logic gốc hay cấu trúc thư mục của bạn.
