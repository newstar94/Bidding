...
def test_metrics_include_wal_disk_backup_restore_and_pool_timing(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(helpers.database, "backend_name", "sqlite")
    monkeypatch.setattr(
        helpers.database, "db_path", str(database_path), raising=False
    )
...
def test_metrics_expose_bounded_postgresql_pool_state(monkeypatch):
    database = SimpleNamespace(
        ...
        pool_stats=lambda: {
            ...
            "acquire_count": 12,
            "acquire_timeouts": 1,
            "acquire_wait_seconds": 0.75,
            "transaction_count": 9,
            "transaction_seconds": 1.25,
        },
    )
    ...
    assert 'biddingflow_postgresql_pool_connections{state="in_use"} 4' in output
    assert "biddingflow_postgresql_pool_waiting 3" in output
    assert "biddingflow_postgresql_pool_acquire_timeouts_total 1" in output
    assert "biddingflow_postgresql_pool_transaction_seconds_total 1.25" in output


def test_postgresql_observability_templates_cover_required_failure_modes():
    alerts_document = yaml.safe_load(
        (
            PROJECT_ROOT
            / "deploy"
            / "prometheus"
            / "biddingflow-alerts.yml"
        ).read_text(encoding="utf-8")
    )
    alert_names = {
        rule["alert"]
        for group in alerts_document["groups"]
        for rule in group["rules"]
        if "alert" in rule
    }
    assert {
        "BiddingFlowPostgreSQLExporterDown",
        "BiddingFlowPostgreSQLPoolWaiting",
        "BiddingFlowPostgreSQLPoolAcquireTimeout",
        "BiddingFlowPostgreSQLConnectionsHigh",
        "BiddingFlowPostgreSQLDeadlock",
        "BiddingFlowPostgreSQLReplicationLag",
        "BiddingFlowPostgreSQLArchiveFailure",
    } <= alert_names

    scrape_document = yaml.safe_load(
        (
            PROJECT_ROOT
            / "deploy"
            / "prometheus"
            / "biddingflow-scrape.yml"
        ).read_text(encoding="utf-8")
    )
    jobs = {job["job_name"] for job in scrape_document["scrape_configs"]}
    assert {"biddingflow", "biddingflow-postgresql"} <= jobs
    ...
    assert {
        ...
        "PostgreSQL deadlocks and replication lag",
    } <= panel_titles
