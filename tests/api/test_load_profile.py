...
def test_runtime_validation_requires_csrf_for_mutating_scenarios(tmp_path):
    profile = load_profile(MIXED_PROFILE)
    sessions_path = tmp_path / "sessions.json"
    sessions_path.write_text(
        json.dumps({
            "sessions": [
                {
                    "cookie": f"session_token=token-{index}",
                    "organizationId": "org",
                    "packageId": "package",
                }
                for index in range(100)
    ...
    with pytest.raises(LoadProfileError, match="csrfToken is required"):
        validate_runtime_inputs(profile, sessions_path=sessions_path)
...
def test_k6_harness_has_target_interlock_and_machine_summary():
    ...
    assert "'X-CSRF-Token': csrfToken" in harness
    assert "csrf_token=${csrfToken}" in harness
