...
test('authenticated reload keeps lazy workflows and Excel actions ready', async ({ page, credentials }) => {
  ...
  const restoredIdentity = await page.evaluate(async () => {
    ...
    return {
      id: controller?.model?.state?.activeuser?.id || '',
      activeRole: controller?.model?.state?.activerole || '',
      canViewInvestor: controller?.moduleForTab?.('chudautu') === 'chudautu'
        && controller?.model?.hasPermission?.(
          controller.model.state.activeuser?.id,
          'chudautu',
          'view'
        )
    };
  });
  expect(restoredIdentity.id).not.toBe('');
  expect(['super_admin', 'manager', 'employee']).toContain(restoredIdentity.activeRole);
  expect(restoredIdentity.canViewInvestor).toBe(true);
