...
test("the modal close button remains circular after component stylesheet splitting", async () => {
  const controls = await readFile(new URL("../../views/css/ui-controls.css", import.meta.url), "utf8");
  const closeButtonRule = controls.match(/\.modal-close\s*\{([^}]*)\}/)?.[1] || "";
  ...
  const closeIconRule = controls.match(/\.modal-close::before\s*\{([^}]*)\}/)?.[1] || "";
...
test("split UI styles consume design tokens instead of literal colors", async () => {
  const files = ["shell", "controls", "dashboard", "landing", "responsive"];
  const styles = await Promise.all(files.map(name =>
    readFile(new URL(`../../views/css/ui-${name}.css`, import.meta.url), "utf8")
  ));

  for (const [index, source] of styles.entries()) {
    assert.doesNotMatch(source, /#[0-9a-f]{3,8}\b|\b(?:rgb|rgba|hsl|hsla)\(/i, `ui-${files[index]}.css contains a literal color`);
  }

  const variables = await readFile(new URL("../../views/css/variables.css", import.meta.url), "utf8");
  for (const token of ["on-brand", "surface-hover", "line-dashboard", "font-size-xs", "font-size-md", "font-size-lg"]) {
