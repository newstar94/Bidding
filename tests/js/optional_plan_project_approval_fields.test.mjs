...
for (const fieldId of ["kh-maduan", "kh-duan"]) {
  test(`${fieldId} project identity is optional and has no required marker`, () => {
...
test("project type toggle never makes project code required", () => {
  assert.doesNotMatch(
    workflow,
    /getElementById\(["']kh-maduan["']\)\.required\s*=\s*isProject/,
  );
});
