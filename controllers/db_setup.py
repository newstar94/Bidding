...
def khoi_tao_va_di_tru_he_thong(database, hash_password_fn, clean_id_fn):
    try:
        ...
        if cursor.fetchone()[0] > 0:
            ...
            for r in cursor.fetchall():
                ...
                org_name_val = d.get('organization_name')
                if org_name_val:
                    orgs = [o.strip() for o in org_name_val.split(',') if o.strip()]
                    for org in orgs:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute("INSERT INTO tai_khoan (id, ten_dang_nhap, mat_khau, ho_ten, vai_tro, email, goi_dich_vu_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
                           (admin_uuid, 'admin', hash_password_fn('123456'), 'Vy Tuấn Dương', 'super_admin', 'tuanduong51794@gmail.com', 'diamond'))
            
            org_name_val = 'HTD'
            org_hash_id = "org-" + hashlib.md5(org_name_val.encode('utf-8')).hexdigest()[:16]
            cursor.execute("""
                INSERT OR IGNORE INTO to_chuc (id, ten_to_chuc, quan_ly_id)
                VALUES (?, ?, ?)
            """, (org_hash_id, org_name_val, admin_uuid))
            cursor.execute("""
                INSERT OR IGNORE INTO thanh_vien_to_chuc (user_id, to_chuc_id, vai_tro_trong_to_chuc)
                VALUES (?, ?, ?)
                ...
                for gt_row in gt_rows:
                    ...
                    if td_list_str:
                        try:
                            ...
                            pass
                print("Đồng bộ: Đã di trú chuyên gia từ goi_thau.chuyen_gia_list và tham_dinh_list sang goi_thau_chuyen_gia thành công!")
        except Exception as migration_ex:
            print("Lỗi khi di trú chuyên gia trước khi xóa cột:", migration_ex)
