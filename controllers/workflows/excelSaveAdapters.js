...
export function saveBasicExcelImport(controller, type, validRows) {
    ...
    if (type === 'chudautu') {
        const mappedData = validRows.map(row => {
            ...
            const existing = controller.model.state.chudautu.find(c =>
    ...
    if (type === 'nhathau') {
        const mappedData = validRows.map(row => {
            ...
            const existing = controller.model.state.nhathau.find(n =>
    ...
    if (type === 'chuyengia') {
        const mappedData = validRows.map(row => {
            ...
            const existing = controller.model.state.chuyengia.find(cg =>
...
function ensureContractorForOpeningImport(controller, row) {
    let foundNt = controller.model.state.nhathau.find(n =>
...
function saveAwardResultImport(controller, validRows) {
    ...
    validRows.forEach(row => {
        ...
        if (bid) {
            bid.lyDoTruot = (row.trangThai === 'Trúng thầu' || row.trangThai === 'trung')
                ? ''
                : (row.lyDoTruot || 'Nhà thầu xếp hạng 1 trúng thầu');
        }
    });
    ...
    if (winnerRow) {
        ...
        if (!winnerId) {
            const matchedBid = controller.model.state.thongtinmothau.find(b =>
        ...
        goiThau.giaTrungThau = (winnerRow.giaTrungThau !== undefined && winnerRow.giaTrungThau !== null) ? winnerRow.giaTrungThau : null;
