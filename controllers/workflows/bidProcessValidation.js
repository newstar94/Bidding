export function isDirectOrSpecialPackage(gt) {
    return gt?.hinhThucLuaChon === 'Chỉ định thầu rút gọn'
        || gt?.hinhThucLuaChon === 'Lựa chọn nhà thầu trong trường hợp đặc biệt';
}
...
export function isNextEvaluationStepSaved(gt) {
    ...
    try {
        ...
        return is1G2T
            ? !!(parsed.is1G2T && parsed.technical && parsed.technical.saved)
            : !!parsed.saved;
    } catch (e) {
        return false;
    }
...
export function canSaveOpeningInfo(gt) {
    ...
    return gt.trangThai === 'Đang mời thầu'
        || gt.trangThai === 'Đã mở thầu'
        || (gt.trangThai === 'Đang chấm thầu' && !isNextStepSaved);
}
...
export function validateOpeningTime(gt, formatDeadline) {
    ...
    const isInvalid = !isNaN(dongThauDate.getTime())
        && !isNaN(moThauDate.getTime())
        && moThauDate < dongThauDate;

    if (!isInvalid) {
        return { valid: true, message: '' };
    }

    const deadline = typeof formatDeadline === 'function'
        ? formatDeadline(gt.thoiGianDongThau)
        : gt.thoiGianDongThau;
    return {
        valid: false,
