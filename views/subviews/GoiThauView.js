...
export function renderExcelPreview(rows, importType) {
    ...
    tableBody.innerHTML = rows.map((r, rowIndex) => {
        // Calculate validation status for each field initially
        const rowErrors = [];
        const fieldErrorMap = {};
        keys.forEach(k => {
        ...
        keys.forEach(k => {
            ...
            const isInvalidClass = errorText ? 'is-invalid' : '';

            rowHtml += `<td style="text-align: ${align} !important; ${style}; padding: 4px; vertical-align: top;">
                <input type="${inputType}" class="${inputClass} ${isInvalidClass}" data-key="${k}" value="${val !== undefined && val !== null && val !== '' ? val : ''}" style="text-align: ${align};">
    ...
    // Setup edit listener on inputs using change/blur event
    tableBody.onchange = (e) => {
        ...
        if (window.appController && window.appController._excelImportData) {
            ...
            rowInputs.forEach(inp => {
                ...
                // Remove existing warning feedback
    ...
    // Use event delegation for focus event to select text
    ...
    // Initialize flatpickr on the preview inputs
    if (typeof this.initFlatpickr === 'function') {
        this.initFlatpickr(tableBody);
    }
