...
export function showPackageDetails(id, isSwitchingVersion = false) {
  ...
  if (tabHeadersEl) {
    ...
    tabHeadersEl.innerHTML = tabs.map((t) => {
      ...
      const style = this._currentWorkflowTab === t.id ? "background: var(--bg-card); color: var(--primary); border: 1px solid var(--border-color); border-bottom: 2px solid var(--primary); font-weight: 700;" : "background: transparent; color: var(--text-muted); border: 1px solid transparent; cursor: pointer;";
  ...
  switch (this._currentWorkflowTab) {
                    ...
                    <div class="table-container" style="border: 1px solid var(--border-color); border-radius: var(--radius-md); overflow-x: auto; margin-bottom: 24px;">
                        <table class="data-table" id="opening-fin-table" style="min-width: 100%;">
                            ...
                            <tbody>
                                ...
                                ${qualifiedBidsForOpening2.map((b) => {
    ...
    case "result":
      ...
      const allBidsForResult = this.model.state.thongtinmothau.filter(
      ...
      if (isAwarded) {
        ...
        const allBids = this.model.state.thongtinmothau.filter((b) => String(b.goiThauId) === String(gt.id)).map(bindResultVersion);
        allBids.sort((a, b) => {
        ...
        if (exportBtn) {
          exportBtn.onclick = () => {
            ...
            const dbId = id;
            const headers = {};
            fetch(`/api/export-report/${dbId}?type=result`, { headers }).then((r) => {
        ...
        const kh2 = this.model.getLatestPlan(gt.keHoachId);
        const cdt = kh2 ? this.model.state.chudautu.find((c) => c.id === kh2.chuDauTuId) : null;
        ...
        contentWrapper.innerHTML = `
                    <div style="padding: 12px 16px; background: rgba(59, 130, 246, 0.05); border: 1px solid rgba(59, 130, 246, 0.15); border-radius: var(--radius-md); font-size: 0.85rem; color: var(--text-main); line-height: 1.6; margin-bottom: 24px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 8px; font-size: 0.82rem;">
                            ...
                            ${!isDirectOrSpecial ? `
                            <div>• <strong style="color: var(--primary);">Thời gian đóng thầu:</strong> ${gt.thoiGianDongThau ? this.model.formatDateWithTime(gt.thoiGianDongThau) : "--"}</div>
                            <div>• <strong style="color: var(--primary);">${is1G2T2 ? "Thời gian mở E-HSĐXKT" : "Thời gian mở thầu"}:</strong> ${gt.thoiGianMoThau ? this.model.formatDateWithTime(gt.thoiGianMoThau) : "--"}</div>
                            ${is1G2T2 ? `<div>• <strong style="color: var(--primary);">Thời gian mở E-HSĐXTC:</strong> ${gt.thoiGianMoEhsdxtc ? this.model.formatDateWithTime(gt.thoiGianMoEhsdxtc) : "Chưa mở"}</div>` : ""}
                            ` : ""}
                        </div>
                    </div>
        ...
        const addCdtrugRow = (bidData = {}) => {
          ...
          [".cdtrug-gia-du-thau", ".cdtrug-gia-tri-dam-bao"].forEach((cls) => {
            ...
            if (el) el.addEventListener("blur", () => {
              el.value = this.model.formatVND(this.model.parseVND(el.value)) || "";
            });
          });
        ...
        if (approveBtn) {
          approveBtn.onclick = async () => {
            ...
            const resolveResultContractorId = (versionId) => selectContractorVersionForDate(this.model, versionId, decDate)?.id || versionId || "";
            tbodyApprove.querySelectorAll("tr").forEach((tr) => {
            ...
            meta.result.contractorBindings = winnerRows.map((row) => {
              ...
              const bid = this.model.state.thongtinmothau.find((item) => String(item.id) === String(bidId));
              return {
                bidId,
    ...
    case "cancel": {
      ...
      if (btnSaveCancel) {
        btnSaveCancel.onclick = async () => {
          ...
          let meta2 = {};
          try {
            meta2 = gt.danhGiaHsdtMetadata ? JSON.parse(gt.danhGiaHsdtMetadata) : {};
          } catch (e) {
          }
          if (!meta2.cancelDetails) meta2.cancelDetails = {};
          if (!meta2.cancelDetails.trangThaiTruocHuy && gt.trangThai !== "Hủy thầu") {
            meta2.cancelDetails.trangThaiTruocHuy = gt.trangThai;
          }
          meta2.cancelDetails.soQuyetDinhHuyThau = decNo;
          meta2.cancelDetails.ngayQuyetDinhHuyThau = formattedDecDate;
          meta2.cancelDetails.lyDoHuyThau = reason;
          gt.danhGiaHsdtMetadata = JSON.stringify(meta2);
