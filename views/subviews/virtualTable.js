...
export function renderVirtualTable(tbody, rows, renderRow, options = {}) {
    ...
    const renderWindow = () => {
        ...
        const topSpacer = topHeight > 0
            ? `<tr aria-hidden="true" class="virtual-spacer"><td colspan="${colSpan}" style="height:${topHeight}px; padding:0; border:0;"></td></tr>`
            : '';
        const bottomSpacer = bottomHeight > 0
            ? `<tr aria-hidden="true" class="virtual-spacer"><td colspan="${colSpan}" style="height:${bottomHeight}px; padding:0; border:0;"></td></tr>`
            : '';
